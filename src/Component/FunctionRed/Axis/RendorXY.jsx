...
const RendorXY = ({ data, xScaleband, yScale, xScaleLinear }) => {
  const { width, height, margin, widthchart, xzoomrange } = useSelector(
  ...
  const newXAxislinearrange = xScaleLinear.range();
  const newYAxisrange = yScale.range();

  // console.log("New range randorAxis",newXAxisrange,newXAxislinearrange);
  ...
  const ySettings2 = {
    translated: `translate(${
      widthchart + margin.padding_right + margin.padding_left
    }, 0)`,
    scale: yScale,
    orient: "right",
    classd: "y-axis",
